...
public class MainActivity extends Activity {
    ...
    private String signalDeckSkinScriptV2() {
        return "(function(){"
                ...
                + ".signaldeck-skin .webrx-top-container{height:74px!important;max-height:74px!important;overflow:hidden!important;background:linear-gradient(180deg,rgba(5,13,21,.95),rgba(5,13,21,.48))!important;box-shadow:0 10px 28px rgba(0,0,0,.3)!important;}"
                ...
                + ".signaldeck-skin .webrx-top-container [class*=expand],.signaldeck-skin .webrx-top-container [id*=expand],.signaldeck-skin .webrx-top-container [class*=collapse],.signaldeck-skin .webrx-top-container [id*=collapse],.signaldeck-skin .webrx-top-container [class*=toggle],.signaldeck-skin .webrx-top-container [id*=toggle],.signaldeck-skin .webrx-top-container [class*=arrow],.signaldeck-skin .webrx-top-container [id*=arrow]{display:none!important;pointer-events:none!important;}"
                ...
                + "function disableNativeTopDrawer(){var top=document.querySelector('.webrx-top-container');if(top){top.style.height='74px';top.style.maxHeight='74px';top.style.overflow='hidden';var controls=top.querySelectorAll('*');for(var i=0;i<controls.length;i++){var key=((controls[i].className||'')+' '+(controls[i].id||'')).toLowerCase();var r=controls[i].getBoundingClientRect();if((key.indexOf('expand')>=0||key.indexOf('collapse')>=0||key.indexOf('toggle')>=0||key.indexOf('arrow')>=0||key.indexOf('chevron')>=0)&&r.width<140&&r.height<80){controls[i].style.display='none';controls[i].style.pointerEvents='none';}}}var all=document.body?document.body.querySelectorAll('*'):[];for(var j=0;j<all.length;j++){var text=(all[j].textContent||'').replace(/\\s+/g,' ').trim().toLowerCase();if(text==='antena'||text==='antenna'||text.indexOf('autor:')>=0||text.indexOf('author:')>=0){var el=all[j];var candidate=null;while(el&&el!==document.body){var rect=el.getBoundingClientRect();if(rect.width>window.innerWidth*.72&&rect.height>70&&rect.top>=55&&rect.top<window.innerHeight*.7){candidate=el;}el=el.parentElement;}if(candidate){candidate.style.display='none';candidate.setAttribute('data-signaldeck-hidden','true');}}}}"
                ...
                + "ensureReceiverHandle();hideReceiverSections();disableNativeTopDrawer();"
                ...
                + "new MutationObserver(function(){hideReceiverSections();disableNativeTopDrawer();ensureReceiverHandle();}).observe(document.body,{childList:true,subtree:true});"
                + "})();";
    }
