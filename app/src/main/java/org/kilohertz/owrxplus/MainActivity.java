...
public class MainActivity extends Activity {
        ...
        panel.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if (event.getActionMasked() == MotionEvent.ACTION_UP) {
                    ...
                    if ((dx > dp(46) && Math.abs(dy) < dp(90)) || dy > dp(36)) {
                        setDeckExpanded(false);
                        return true;
                    }
                }
    ...
    private String signalDeckSkinScriptV2() {
        return "(function(){"
                ...
                + ".signaldeck-skin #signaldeck-waterfall-controls{display:none;margin:9px 0 0!important;padding:8px 9px!important;border:1px solid rgba(159,234,255,.18)!important;background:rgba(5,13,21,.44)!important;border-radius:8px!important;box-sizing:border-box!important;}"
                ...
                + "function markDecoderOutputs(){var nodes=document.body?document.body.querySelectorAll('div,section,article'):[];var terms=/\\b(fax|sstv|tpms|ism|decoded|decoder|packets?|callsign|message|device|pressure_kpa|temperature_c)\\b/i;for(var i=0;i<nodes.length;i++){var el=nodes[i];if(el.closest&&el.closest('#openwebrx-panel-receiver,#signaldeck-receiver-modal,.webrx-top-container,.openwebrx-main-buttons')){continue;}var txt=((el.innerText||el.textContent||'')+'').replace(/\\s+/g,' ').trim();if(!terms.test(txt)){continue;}var r=el.getBoundingClientRect();if(!r||r.width<window.innerWidth*.48||r.height<52||r.top<80){continue;}var box=el;for(var p=el.parentElement;p&&p!==document.body;p=p.parentElement){if(p.closest&&p.closest('#openwebrx-panel-receiver,.webrx-top-container')){break;}var pr=p.getBoundingClientRect();if(!pr||pr.width<=0||pr.height<=0){break;}if(pr.width>=window.innerWidth*.65&&pr.height>=r.height&&pr.height<window.innerHeight*.62){box=p;}if(pr.width>window.innerWidth*.98||pr.height>window.innerHeight*.72){break;}}box.setAttribute('data-signaldeck-decoder-output','true');}}"
                ...
                + "hideForeignPanels();hideReceiverSections();tidyDigRow();ensureWaterfallControls();ensureDefaultStep();markDecoderTables();markDecoderOutputs();hideNativeImageExpander();hideNativeStatusMeters();installReceiverSwipe();ensureReceiverTab();setInterval(ensureWaterfallControls,900);setInterval(ensureDefaultStep,900);setInterval(function(){markDecoderTables();markDecoderOutputs();},1500);"
                + "new MutationObserver(function(){hideForeignPanels();hideReceiverSections();tidyDigRow();markDecoderTables();markDecoderOutputs();hideNativeImageExpander();hideNativeStatusMeters();installReceiverSwipe();ensureReceiverTab();}).observe(document.body,{childList:true,subtree:true});"
                + "})();";
    }
    ...
    private FrameLayout.LayoutParams collapsedPanelParams() {
        ...
        params.setMargins(0, 0, 0, Math.max(dp(128), safeBottomInset + dp(128)));
        return params;
    }
